...
func optionalStringIntoRustString<S: IntoRustString>(_ string: S?) -> RustString? {
  if let val = string {
...
extension String: ToRustStr {
  ...
  public func toRustStr<T>(_ withUnsafeRustStr: (RustStr) throws -> T) rethrows -> T {
    let cString = Array(self.utf8) + [0]
    return try cString.withUnsafeBufferPointer({ bufferPtr in
      let rustStr = RustStr(
        start: UnsafeMutableRawPointer(mutating: bufferPtr.baseAddress!).assumingMemoryBound(
          to: UInt8.self),
...
public class RustVec<T: Vectorizable> {
  ...
  public func get(index: UInt) -> T.SelfRef? {
...
extension UnsafeBufferPointer {
  func toFfiSlice() -> __private__FfiSlice {
    __private__FfiSlice(
      start: UnsafeMutablePointer(mutating: self.baseAddress), len: UInt(self.count))
  }
}
